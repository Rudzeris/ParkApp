...
- [x] Оболочка с вкладками: разделы и редакторы открываются вкладками, вкладку можно вытащить мышью в отдельное окно, окна независимы (`ShellViewModel`, `DocumentViewModel`, `ShellWindow`).
- [x] Экраны переведены с `Window` на `UserControl` — одна разметка и во вкладке, и в окне.
- [x] Поиск с тремя способами записи запроса (`SearchQuery`): в кавычках, со звёздочкой, по похожим словам. Проверка — `tools/search-check.py`.
...
- [x] Двойной щелчок по строке открывает запись на изменение; правая кнопка — меню «Изменить / Удалить / Открыть скан».
- [x] Поиск машины в редакторе: список сокращается по мере ввода, ищется по номеру, VIN и обоим написаниям марки.
