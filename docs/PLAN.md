...
- [ ] Закрытые списки для «Куда относится» и «Тип машины», когда придут перечни значений (О-5, О-11).
...
- [ ] Наработка (пройдено км) — столбца в таблице заказчика нет, а требования его называют: уточнить, откуда брать.
...
- [ ] Включение разделов через `App.config`: кто не работает со штрафами, не должен видеть кнопку (О-12).
