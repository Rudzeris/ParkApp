...
- [ ] Получить реальный файл «Машины.xlsx» и сверить названия столбцов с синонимами в `ExcelCarRepository` (О-7).
- [ ] Дополнить `Car`: `Pfm`, `Pts`, `RegCertificate`, `PowerKw`, `PowerHp`, `Affiliation` (Гараж/Обеспечение), `Odometer`.
- [ ] Разбор мощности «220/299» в два числа при чтении и сборка обратно при записи.
- [ ] **Запись машин в Excel с сохранением неизвестных столбцов**: приложение правит только те ячейки, которые понимает, остальные переносит как есть — иначе перезапись потеряет столбцы, которых нет в модели.
...
- [ ] `MaintenanceRequest` + репозиторий в Excel, вложение скана заявки.
...
| Файл машин в старом формате `.xls` | не читается | пересохранить в Excel как `.xlsx` (О-8) |
