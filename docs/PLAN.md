...
- [x] В строке набирают только время: группа, цель, маршрут, «в чьё распоряжение» и примечание выбираются из списков (`DispatchChoices`, лист «Списки»); значения, уже стоящие в данных, в список добавляются сами.
...
| В списках наряда не хватает нужного значения | его негде выбрать, работа встала | список правится в Excel на листе «Списки» и подхватывается при следующем открытии окна; значения из данных попадают в список сами |
