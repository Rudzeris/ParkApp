...
- **в строке ничего не набирают, кроме времени.** Группа, группа эксплуатации, цель, маршрут, «в чьё распоряжение» и примечание выбираются из выпадающих списков. Причина не в удобстве, а в том, что набранное руками расходится опечатками: «Перевозка л/с», «перевозка ЛС» и «Перевозка личного состава» — три разных значения, которые потом не сходятся ни в наряде, ни в путевом листе, ни в отчёте. Списки ведёт пользователь в Excel, на листе «Списки» файла нарядов — по столбцу на список;
- **список дополняется тем, что уже стоит в данных.** Значение, записанное до появления списков или правкой файла руками, обязано в списке оказаться — иначе выбор «только из списка» молча затёр бы его. Первый пункт каждого списка — «—», им значение очищают;
...
| Application | `DispatchChoices.cs` — списки для выбора в наряде |
| Инструменты | `tools/build-templates.py` — сборка шаблонов из подлинных форм; `tools/render-check.py` — проверка заполнения без Visual Studio |
...
- в строке набирают только время — остальное выбирается из списков, которые ведутся на листе «Списки»;
...
3. На листе **«Списки»** поправить значения, которые будут предлагаться в наряде: группы, цели, маршруты, «в чьё распоряжение». По столбцу на список.
