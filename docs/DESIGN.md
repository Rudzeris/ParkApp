...
| Куда относится | `Affiliation` | string | Гараж, обеспечение и т.д. — **см. О-5** |
...
| Тип машины | `VehicleType` | string | легковой седан/универсал, автобус — **см. О-11** |
| Цвет | `Color` | string | |
...
«Куда относится» и «Тип машины» помечены заказчиком как enum, но полный список значений пока не задан, поэтому в модели это **строки**. Причина: enum молча потеряет значение, которого в нём нет, а строка сохранит данные из файла как есть. Заводим закрытые списки, когда получим перечень (О-5, О-11).
...
| **О-3** | Водитель в штрафе — строка ФИО или ссылка на справочник людей? | справочник людей теперь есть; ссылка убрала бы разнобой в ФИО | предложено перевести на `Person.Id` с выбором из списка; сейчас строка с автодополнением |
...
| **О-5** | «Куда относится» — полный список значений | сейчас читается строкой; закрытый список даст фильтры и защиту от опечаток | прислать перечень (Гараж, обеспечение, …) |
| **О-6** | Один водитель на путевой лист или несколько (смена)? | структура путевого листа | предложен один |
| **О-8** | Файл машин в `.xlsx` или в старом `.xls`? | `.xls` читается только после пересохранения | пересохранить как «Книга Excel (*.xlsx)» |
...
| **О-11** | «Тип машины» — полный список значений | то же, что О-5 | прислать перечень (легковой седан, универсал, автобус, грузовой, …) |
| **О-12** | Нужно ли скрывать разделы, которыми человек не пользуется | «я не работаю со штрафами» — лишние кнопки мешают | предложено: список включённых разделов в `App.config` |
...
| Application | `ScanCategory.cs`, `IPersonRepository.cs`, `PersonService.cs` |
| Infrastructure | `ExcelFineRepository.cs`, `ExcelCarRepository.cs`, `ExcelPersonRepository.cs`, `FileScanStorage.cs`, `WpfFileDialogService.cs`, `AppPaths.cs` |
| Infrastructure/Excel | `XlsxReader.cs`, `XlsxWriter.cs`, `XlsxCell.cs`, `SheetTable.cs` — чтение и запись .xlsx без внешних библиотек |
| ViewModel | `FineListViewModel.cs`, `FineEditViewModel.cs`, `FineRowViewModel.cs`, `CarListViewModel.cs`, `CarRowViewModel.cs`, `CarOption.cs`, `RelayCommand.cs`, `IFineDialogService.cs` |
