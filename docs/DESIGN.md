...
Правило: **заполненная дата оплаты сама по себе означает «оплачен»**. Это избавляет от рассинхрона, когда дату проставили, а галку забыли; в редакторе отметка ставится автоматически при выборе даты.
...
| `IsPaid` | bool | да | оплачен |
| `PaidDate` | DateTime? | нет | дата оплаты; может быть пустой и у оплаченного — отметку ставят задним числом |
...
**Выбранный файл проверяется сразу.** Пользователь должен узнать «это не тот файл» при выборе, а не при следующем открытии окна. Проверяются три вещи: расширение `.xlsx`, наличие нужного листа («Машины», «Люди», «Штрафы») и обязательного столбца-ключа (`VIN`, `ФИО`, `№ постановления`). Если что-то не так — предупреждение с причиной и **повторный запрос файла**, пока файл не подойдёт или пользователь не откажется. Путь, вписанный руками, проверяется при сохранении: непригодный файл не даст закрыть окно.
...
| **О-4** | Статус оплаты штрафа | **нужен**: «Оплачен» и «Дата оплаты» (может быть пустой) |
